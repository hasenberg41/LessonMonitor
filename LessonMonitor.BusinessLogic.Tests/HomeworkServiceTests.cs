...
namespace LessonMonitor.BusinessLogic.Tests
{
    public class HomeworkServiceTests
    {
        ...
        public async Task Create_ShouldCreateNewHomework()
        {
            ...
            // ASSERT
            Assert.IsTrue(result == homework.Id);
            _homeworkRepositoryMock.Verify(x => x.Add(It.IsAny<HomeWork>()), Times.Once);
        }
        ...
        [Test]
        public void Create_HomeWorkIsInValid_ShouldThrowBusinesException(string link, string title)
        {
            // ARRANGE
